...
def _get_owner_participant(memoir_id: UUID, user_id: UUID) -> dict:
    ...
    if not result or not result.data:
        raise PermissionError("User is not an active owner or co-owner of this memoir.")
    return result.data
...
def confirm_media(memoir_id: UUID, memory_id: UUID, user_id: UUID, request: MediaConfirmRequest) -> MediaAssetOut:
    ...
    if not request.storage_key.startswith(f"{memoir_id}/{request.kind.value}/"):
        raise ValueError("Invalid storage key format.")

    if request.kind == MediaKind.audio and not request.duration_ms:
        raise ValueError("duration_ms is required for audio assets.")
...
def remove_media(memoir_id: UUID, memory_id: UUID, media_asset_id: UUID, user_id: UUID) -> None:
    ...
    links = client.table("memory_media").select("id").eq("media_asset_id", str(media_asset_id)).execute()
    if not links.data:
        now_iso = datetime.now(timezone.utc).isoformat()
