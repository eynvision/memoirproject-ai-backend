...
from src.models.memory_models import (
    MemoryOut, MemoryListResponse,
    MemoryCreateRequest, MemoryPatchRequest,
    PresignRequest, PresignResponse,
    MediaConfirmRequest, MediaAssetOut,
)
...
async def list_memories(
    memoir_id: UUID, include_drafts: bool = True, user_id: UUID = Depends(get_current_user_id)
):
    try:
        memories, total = memory_service.list_memories(memoir_id, user_id, include_drafts=include_drafts)
...
async def create_memory(
    memoir_id: UUID, request: MemoryCreateRequest, user_id: UUID = Depends(get_current_user_id)
):
    try:
...
async def patch_memory(
    memoir_id: UUID, memory_id: UUID, patch: MemoryPatchRequest, user_id: UUID = Depends(get_current_user_id)
):
    try:
...
async def submit_memory(
    ...
    try:
        return memory_service.submit_memory(memory_id, memoir_id, user_id)
    except LookupError as e:
        raise HTTPException(status.HTTP_404_NOT_FOUND, str(e))
    except PermissionError as e:
        raise HTTPException(status.HTTP_403_FORBIDDEN, str(e))
...
async def presign_media_upload(
    memoir_id: UUID, memory_id: UUID, request: PresignRequest, user_id: UUID = Depends(get_current_user_id)
):
    try:
...
@router.post("/{memory_id}/media/confirm", response_model=MediaAssetOut, status_code=status.HTTP_201_CREATED)
async def confirm_media_upload(
    memoir_id: UUID, memory_id: UUID, request: MediaConfirmRequest, user_id: UUID = Depends(get_current_user_id)
):
    try:
...
async def remove_media(
    memoir_id: UUID, memory_id: UUID, media_asset_id: UUID, user_id: UUID = Depends(get_current_user_id)
):
    try:
